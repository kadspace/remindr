...
class ReminderReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // Robust DB Check
        val driverFactory = com.kizitonwose.remindr.compose.multiplatform.sample.DatabaseDriverFactory(context)
        val database = com.kizitonwose.remindr.sample.db.RemindrDatabase(driverFactory.createDriver())
        val dbHelper = com.kizitonwose.remindr.compose.multiplatform.sample.NoteDbHelper(database)
    ...
    private fun showNotification(context: Context, note: com.kizitonwose.remindr.compose.multiplatform.sample.CalendarNote) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val importance = when(note.severity) {
                com.kizitonwose.remindr.compose.multiplatform.sample.Severity.HIGH -> NotificationManager.IMPORTANCE_HIGH
                com.kizitonwose.remindr.compose.multiplatform.sample.Severity.LOW -> NotificationManager.IMPORTANCE_LOW
        ...
        val color = when(note.severity) {
            com.kizitonwose.remindr.compose.multiplatform.sample.Severity.HIGH -> android.graphics.Color.RED
            com.kizitonwose.remindr.compose.multiplatform.sample.Severity.MEDIUM -> android.graphics.Color.YELLOW
            com.kizitonwose.remindr.compose.multiplatform.sample.Severity.LOW -> android.graphics.Color.BLUE
        }
